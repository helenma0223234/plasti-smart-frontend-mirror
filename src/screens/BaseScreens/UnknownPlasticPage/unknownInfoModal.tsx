...
import ReuseWarningModal from './reuseWarningModal';


type UnknownInfoModalProps = {
  navigation: StackNavigationProp<BaseNavigationList>;
  plasticNumber: number;
  setThisModalVisible: (visible: boolean) => void;
  thisModalVisible: boolean;
};
...
const UnknownInfoModal = ({ navigation,  plasticNumber, setThisModalVisible, thisModalVisible }: UnknownInfoModalProps ) => {
  ...
  const plasticMessages = {
    ...
    eighth: 'This polymer is not recycled. Check labels to confirm.',
  ...
  return (
    <Modal
      animationType="slide"
      transparent={false}
      visible={thisModalVisible}
      onRequestClose={() => {
        setThisModalVisible(!thisModalVisible);
      }}
    >
...
export default UnknownInfoModal;